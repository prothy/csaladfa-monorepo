...
import type { SeederOptions } from './seederOptions';
...
function _get(
  data: Member[],
  ids: number[],
  options: { preserveParentIds: boolean },
): Member[] {
...
function _getChildren(data: Member[], ...parents: Member[]): Member[] {
  const childIds = data
    .filter((member) =>
      parents.some(
        (parent) =>
          parent.id === member.parent1Id || parent.id === member.parent2Id,
      ),
    )
  ...
  const parentDepthOffset = parents.find(
    (parent) => parent.depthOffset !== undefined,
  )?.depthOffset;
...
function _getOtherParents(
  ...
  const uniqueOtherParentIds = otherParentIds.filter(
    (value, index) => index === otherParentIds.indexOf(value),
  );

  // remove parentIds so their ancestors aren't included
  const otherParents = _get(data, uniqueOtherParentIds as number[], {
  ...
  if (parentDepthOffset !== undefined) {
    otherParents.forEach(
      (otherParent) => (otherParent.depthOffset = parentDepthOffset),
    );
  }
  return otherParents;
}
function _getRelatives(data: Member[], targetId?: number): Member[] {
  ...
  if (!hasParent1 && !hasParent2) {
    ...
    const siblingIds = data
      .filter(
        (member) =>
          (member.parent1Id === target.parent1Id ||
            member.parent2Id === target.parent2Id ||
            member.parent1Id === target.parent2Id ||
            member.parent2Id === target.parent1Id) &&
          member.id !== target.id,
      )
      .map((member) => member.id);
  ...
  do {
    ...
    const nextGenerationOtherParents = _getOtherParents(
      data,
...
function _combineIntoMarriages(
  data: Member[],
  ...
  parentGroups.forEach((currentParentGroup) => {
    ...
    const nodeMarriages = parentGroups.filter((group) =>
    ...
    nodeMarriages.forEach((marriedCouple) => {
      ...
      if (spouseId !== undefined) {
        marriage.spouse = new TreeNode(
          _getWithParentIds(data, spouseId),
          options,
        );
      }
      ...
      marriage.children = data
        .filter((member) => {
          if (member.parent1Id !== null && member.parent2Id !== null) {
            return (
              marriedCouple.includes(member.parent1Id as number) &&
...
function _coalesce(data: TreeNode[]): TreeNode[] {
  ...
  while (data.length > 1) {
    for (let index = 0; index < data.length; index++) {
      ...
      if (
        otherNodes.some((otherNode) => otherNode.canInsertAsDescendant(node))
      ) {
        data.splice(index, 1);
      }
    }
    ...
    if (count > _generationLimit) {
      throw new Error(
        `Data contains multiple roots or spans more than ${_generationLimit} generations.`,
      );
    }
  }
  return data;
