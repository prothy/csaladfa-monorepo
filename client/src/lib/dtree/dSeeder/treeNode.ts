...
export class TreeNode {
  ...
  canInsertAsDescendant(descendent: TreeNode): boolean {
    ...
    for (let index = 0; index < this.marriages.length; index++) {
      ...
      if (
        this._canInsertAsDescendantByPivotingOnAMarriage(marriage, descendent)
      ) {
        return true;
      }

      if (
        marriage.children.some((child) =>
          child.canInsertAsDescendant(descendent),
        )
      ) {
        return true;
      }
  ...
  private _overwriteChild(
    marriage: TreeNodeMarriage,
    newChild: TreeNode,
  ): void {
    const index = marriage.children.findIndex(
      (child) => child.id === newChild.id,
    );
    marriage.children.splice(index, 1, newChild);
  }
  private _canInsertAsDescendantByPivotingOnAMarriage(
    marriage: TreeNodeMarriage,
    descendent: TreeNode,
  ): boolean {
    ...
    const marriageToPivotOn = descendent.marriages.find((m) =>
      childIds.includes(m.spouse?.id as number),
    );
    const newDescendent = this._pivotOnMarriage(
      marriageToPivotOn as TreeNodeMarriage,
      descendent,
    );
